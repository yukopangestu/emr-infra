...
export const architectureData: ArchitectureData = {
  ...
  sections: [
    ...
    },
    {
      ...
      content: 'Trafik pasien masuk lewat Cloudflare menuju ALB Public dan Gateway Public, dan hanya dapat mencapai layanan yang dibutuhkan pasien. Trafik staf masuk lewat Cloudflare menuju ALB Admin dan Gateway Admin dengan login IdP + MFA. Akses operator platform (runbook, break-glass) terpisah lewat Cloudflare Zero Trust. DNS publik dan proxy ada di Cloudflare (paket Pro mewajibkan Cloudflare sebagai DNS authoritative). Route 53 hanya memegang zona origin dan health check; ALB origin dikunci sehingga akses langsung di luar Cloudflare ditolak.\n\nPgBouncer berjalan sebagai dua replika workload EKS di private app subnet. Security group RDS (primary dan read replica) hanya menerima koneksi dari PgBouncer; Reporting memakai pool terpisah ke read replica. Zona data tidak memiliki rute ke internet; egress ke SATUSEHAT dan BPJS lewat NAT per AZ.',
      diagram: {
